...
pub(crate) enum DonationOutcome {
...
pub(crate) fn donation_outcome(count_after_increment: f32, bound: i32) -> DonationOutcome {
...
pub(crate) fn describe_calculate_sums_fixture() -> String {
...
///   poke from here.
pub(crate) fn fixup_calc_sums_fixture() -> String {
    let base = get_module_base("zoo.exe") as u32;
    let need_threshold: i32 = get_from_memory(base + raw_globals::GUEST_NEED_THRESHOLD_RVA);
...
impl ZooStatus {
    ...
    pub(crate) fn price_tier(price: f32, boundaries: [f32; 4]) -> i32 {
    ...
    pub(crate) fn newguest_dispatch(attendance: i32, price_tier: i32, multipliers: [i32; 5]) -> Option<(i32, bool)> {
        let m = multipliers;
        if attendance >= 0x51 {
            match price_tier {
                1 => Some((m[2], false)),
